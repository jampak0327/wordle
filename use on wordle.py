...
with open('C:/Users/User/Documents/Wordle/valid_solutions.csv', 'r') as guesses:
    csv_reader = csv.reader(guesses)
    list_of_Guesses = list(csv_reader)

with open('C:/Users/User/Documents/Wordle/valid_solutions.csv', 'r') as solutions:
...
def guess(word):
    ...
    if(word==keyWord):
        ...
        for i in range(len(list_of_Guesses)):
            try:
                if(list_of_Guesses[i] != word):
                    list_of_Guesses.remove(list_of_Guesses[i])
            except:
                continue
        print('Used:', count+1, 'tries\n0 Words remaining.')
        quit()
    else:        
...
while(res != keyWord):
    ...
    if(len(list_of_Guesses)==1):
        #res = keyWord
        print('Tries:',count)
        print("There is logically only one answer left.")
        break
    res = "".join(myAnswer)
    if(count==5):
        res = keyWord
